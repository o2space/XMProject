...
extension UIScrollView {
    ...
    */
    ...
    /**
     上拉加载跟多 水平
    */
    public func footerViewPullToRefreshHorizontal(callback : (()->Void)?){
        footerViewPullToRefresh(.XMRefreshDirectionHorizontal, callback:callback)
    }
    
    /**
     上拉加载跟多 竖直
    */
    public func footerViewPullToRefreshVertical(callback : (()->Void)?){
        footerViewPullToRefresh(.XMRefreshDirectionVertical, callback:callback)
    }
    ...
    */
    func footerViewPullToRefresh(direction : XMRefreashDirection, callback : (()->Void)?) {
        ...
        if direction == .XMRefreshDirectionHorizontal {
            ...
            footView = XMRefreshFooterView(frame: CGRectMake(0, 0, XMRefreshViewHeight, self.height))
        } else {
            //footView = XMRefreshFooterView(frame: CGRectMake(0, 0, SCREEN_WIDTH, XMRefreshViewHeight))
            footView = XMRefreshFooterView(frame: CGRectMake(0, 0, self.width, XMRefreshViewHeight))
        }
