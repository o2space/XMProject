...
extension UILabel  {
    ...
    func textWHAlign(width : CGFloat,height : CGFloat, font : UIFont) {
        let style : NSMutableParagraphStyle = NSMutableParagraphStyle()
        style.lineBreakMode = NSLineBreakMode.ByWordWrapping
        
        let attributes = [NSFontAttributeName : font, NSParagraphStyleAttributeName : style.copy()]
        let labelSize = (self.text! as NSString).boundingRectWithSize(CGSize(width: width, height: height), options: .UsesLineFragmentOrigin, attributes: attributes, context: nil)
